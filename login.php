<?php
/**
 * login.php
 * Handles login form display and server-side validation.
 */

require_once 'includes/validation.php';

// ── Handle POST submission ──────────────────────────────────────────────────
$errors   = [];
$success  = false;
$username = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $username = getInput('username');
    $password = getRawInput('password');

    $errors = validateLogin($username, $password);

    if (empty($errors)) {
        $success = true;
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>UM Skills Clinic — Login</title>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/auth.css">
</head>
<body>

    <!-- Background Layers -->
    <div class="bg"></div>
    <div class="bg-grain"></div>
    <div class="bg-shimmer"></div>

    <!-- Login Card -->
    <div class="card-wrap">
        <div class="card">

            <!-- Logo -->
            <div class="card__logo">
                <img class="card__logo-img" src="umsclogo.png" alt="UM Skills Clinic Logo">
            </div>

            <h1 class="card__title">Welcome to UM Skills Clinic</h1>
            <p class="card__subtitle">Please log in using the form below.</p>

            <!-- Error Alert -->
            <?php if (!empty($errors)): ?>
                <div class="alert alert--error alert--visible">
                    ⚠ <?= htmlspecialchars($errors[0]) ?>
                </div>
            <?php endif; ?>

            <!-- Success Alert -->
            <?php if ($success): ?>
                <div class="alert alert--success alert--visible">
                    ✓ Login successful! Welcome back, <strong><?= htmlspecialchars($username) ?></strong>.
                </div>
            <?php endif; ?>

            <!-- Login Form (hidden on success) -->
            <?php if (!$success): ?>
                <form method="POST" action="" novalidate>

                    <!-- Username -->
                    <div class="field">
                        <div class="field__header">
                            <label class="field__label" for="username">Username</label>
                        </div>
                        <div class="input-wrap">
                            <span class="input-wrap__icon-left">
                                <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
                                    <circle cx="12" cy="7" r="4"/>
                                </svg>
                            </span>
                            <input
                                type="text"
                                id="username"
                                name="username"
                                class="input"
                                placeholder="Enter your username"
                                value="<?= htmlspecialchars($username) ?>"
                                autocomplete="username"
                            >
                        </div>
                    </div>

                    <!-- Password -->
                    <div class="field">
                        <div class="field__header">
                            <label class="field__label" for="password">Password</label>
                            <a href="#" class="field__link">Forgot password?</a>
                        </div>
                        <div class="input-wrap">
                            <span class="input-wrap__icon-left">
                                <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <rect x="3" y="11" width="18" height="11" rx="2"/>
                                    <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                                </svg>
                            </span>
                            <input
                                type="password"
                                id="password"
                                name="password"
                                class="input"
                                placeholder="Enter your password"
                                autocomplete="current-password"
                            >
                            <span class="input-wrap__icon-right" onclick="togglePasswordVisibility('password', 'eye-icon')" title="Toggle password visibility">
                                <svg id="eye-icon" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                    <circle cx="12" cy="12" r="3"/>
                                </svg>
                            </span>
                        </div>
                    </div>

                    <!-- Remember Me -->
                    <div class="remember-row">
                        <input type="checkbox" id="remember" name="remember">
                        <label for="remember">Remember me</label>
                    </div>

                    <button type="submit" class="btn-primary">Log In</button>

                </form>
            <?php endif; ?>

            <!-- Social Login Divider -->
            <div class="divider">
                <div class="divider__line"></div>
                <span class="divider__text">Or</span>
                <div class="divider__line"></div>
            </div>

            <!-- Google -->
            <a href="#" class="btn-social">
                <span class="btn-social__icon">
                    <svg width="18" height="18" viewBox="0 0 24 24">
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                    </svg>
                </span>
                Continue with Google
            </a>

            <!-- Apple -->
            <a href="#" class="btn-social">
                <span class="btn-social__icon">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                    </svg>
                </span>
                Continue with Apple
            </a>

            <!-- Footer Link -->
            <div class="card__footer">
                Don't have an account? <a href="register.php">Sign Up</a>
            </div>

        </div>
    </div>

    <script src="assets/js/auth.js"></script>

</body>
</html>